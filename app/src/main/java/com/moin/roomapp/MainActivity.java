...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == ADD_NOTE_REQUEST && resultCode == RESULT_OK) {
            ...
            long timestamp = data.getLongExtra(NoteActivity.EXTRA_TIMESTAMP, 1);

            Note node = new Note(title, description, timestamp);
            ...
            String title = data.getStringExtra(NoteActivity.EXTRA_TITLE);
            String description = data.getStringExtra(NoteActivity.EXTRA_DESCRIPTION);
            long timestamp = data.getIntExtra(NoteActivity.EXTRA_TIMESTAMP, 1);

            Note node = new Note(title, description, timestamp);
            node.setId(id);
            noteViewModel.update(node);
